...
use crate::{
    ...
    jwt::{
        context::{Audience, JwtClaims, JwtRequestContext, TokenPayload},
...
impl JwtAuthRuntime {
    ...
    fn authenticate(
        ...
        jwks: &Vec<Arc<JwkSet>>,
        req: &HttpRequest,
    ) -> Result<(TokenPayload, String), JwtError> {
    ...
    fn decode_and_validate_token(
        ...
        Err(JwtError::AllProvidersFailedToDecode(
            decode_attempts
                .into_iter()
                .map(|result: Result<TokenPayload, JwtError>| result.unwrap_err())
                .collect::<Vec<_>>(),
        ))
    }
    ...
    pub fn validate_request(&self, request: &mut HttpRequest) -> Result<(), JwtError> {
        ...
        match self.authenticate(&valid_jwks, request) {
            Ok((token_data, token)) => {
                if let Some(request_ctx) = request.extensions_mut().get_mut::<RequestContext>() {
                    request_ctx.jwt = Some(JwtRequestContext {
                        token,
                        payload: token_data,
                    });
                }
