...
mod stored_size_tests {
    ...
    ///
    /// `FetchNode` is 16 B wider than an inline `SelectionSet` would make it, because
    /// `RequiresSelectionSet` is two boxed slices and a length rather than one `Vec`. That is paid
    /// once per entity fetch and buys back several times as much heap; see the commit that
    /// introduced it.
    #[test]
