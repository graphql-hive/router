...
mod test {
    ...
    fn test_strip_schema_internals() {
        use crate::query_planner::consumer_schema::strip_schema_internals::StripSchemaInternals;
...
type Product implements ProductItf & SkuItf
  ...
  hidden: String @join__field(graph: PRODUCTS) @requiresScopes(scopes: ["read", "admin"])
...
enum ShippingClass @join__type(graph: INVENTORY) @join__type(graph: PRODUCTS) {
  ...
  "#;
        ...
        let schema = graphql_tools::parser::parse_schema(schema).unwrap();
