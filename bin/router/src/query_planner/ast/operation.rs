...
/// An executable fetch keeps its text and has no parsed document.
#[derive(Debug, Clone, Deserialize)]
pub struct SubgraphFetchOperation {
    /// Written once. A box avoids keeping unused string capacity.
...
/// A fetch operation while its parsed document is still needed by the planner.
#[derive(Debug, Clone, Deserialize)]
...
mod tests {
    ...
    impl<'a> InsertPosition<'a> {
        fn new(operation: &'a PlanningFetchOperation) -> Self {
            Self {
                document: &operation.operation.document_str,
                start: operation.operation.name_write_position,
            }
        }
    }
