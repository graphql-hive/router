...
mod type_authorization {
    ...
    mod interfaces {
        ...
        #[test]
        fn allows_typename_on_unauthorized_interface() {
            let supergraph_data = build_supergraph_data(SECURED_INTERFACE_TYPE_SCHEMA);
            let query = r#"
              query($id: ID!) {
                node(id: $id) {
    ...
    mod unions {
       ...
       "#;
        ...
        #[test]
        fn allows_typename_on_unauthorized_union_members() {
        ...
        #[test]
        fn allows_authorized_union_member_removes_unauthorized() {
            let supergraph_data = build_supergraph_data(UNION_SCHEMA);
            let query = "
              query {
                media {
           ...
           ";
            ...
            insta::assert_snapshot!(decision, @r#"
            [Modified]
            Operation: query{media{...on Book{title}}}
            Errors:    ["media"]
            "#);
        }
    }
