...
impl Telemetry {
    ...
    pub async fn graceful_shutdown(&self) {
        debug!(target: "telemetry", "flushing telemetry data");
        ...
        let shutdown_tracer = spawn_blocking(|| {
            if let Some(provider) = tracer {
                tracing::debug!(target: "telemetry", "telemetry trace provider shutdown scheduled");
                let _ = provider.force_flush();
                let _ = provider.shutdown();
                tracing::info!(target: "telemetry", "telemetry trace provider shutdown completed");
            }
        });

        let shutdown_prometheus = async {
            if let Some(runtime) = &self.prometheus {
                tracing::debug!(target: "telemetry", "telemetry prometheus shutdown scheduled");
                runtime.shutdown().await;
                tracing::info!(target: "telemetry", "telemetry prometheus shutdown completed");
            }
        };
