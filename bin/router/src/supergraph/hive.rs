...
impl SupergraphLoader for SupergraphHiveConsoleLoader {
    async fn load(&self) -> Result<ReloadSupergraphResult, LoadSupergraphError> {
        ...
        match fetcher_result {
            ...
            Err(err) => {
                error!(target: "supergraph.hive", error = %err, "Error fetching supergraph from Hive Console");
                Err(LoadSupergraphError::from(err))
            }
            // If the supergraph has not changed, return Unchanged
            Ok(None) => {
                debug!(target: "supergraph.hive", "Supergraph from Hive Console loaded but was not changed");
                Ok(ReloadSupergraphResult::Unchanged)
            }
            // If there is a new supergraph SDL, return it
            Ok(Some(sdl)) => {
                info!(target: "supergraph.hive", "Supergraph from Hive Console loaded and changed");
                debug!(target: "supergraph.hive", sdl, "New supergraph SDL");
