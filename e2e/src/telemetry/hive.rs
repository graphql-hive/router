...
async fn test_hive_http_export() {
    ...
    assert_eq!(
        first_request_spans.by_hive_kind("http.server").len(),
        0,
...
/// Verify Hive Console exporter works with gRPC protocol
#[ntex::test]
async fn test_hive_grpc_export() {
    let supergraph_path =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("supergraph.graphql");
