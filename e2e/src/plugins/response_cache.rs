...
impl RouterPlugin for ResponseCachePlugin {
    ...
    fn on_supergraph_reload<'a>(
        ...
        payload.new_ast.definitions.iter().for_each(|def| {
            if let graphql_tools::parser::schema::Definition::TypeDefinition(type_def) = def {
                if let graphql_tools::parser::schema::TypeDefinition::Object(obj_type) = type_def {
                    for directive in &obj_type.directives {
                        if directive.name == "cacheControl" {
                            for arg in &directive.arguments {
                                if arg.0 == "maxAge" {
                                    if let graphql_tools::parser::query::Value::Int(max_age) =
                                        &arg.1
                                    {
...
mod tests {
    ...
    async fn test_caching_with_default_ttl() {
        ...
        let app = crate::testkit::init_router_from_config_inline_with_plugins(
            r#"
            plugins:
              response_cache_plugin:
                enabled: true
