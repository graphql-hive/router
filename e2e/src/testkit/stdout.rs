...
pub trait CaptureStdoutExt<F: Future>: Sized {
    #[allow(async_fn_in_trait)]
    async fn capture_stdout_lines(self) -> Vec<String>;
    #[allow(async_fn_in_trait)]
    async fn capture_stdout_json(self) -> StdOutCaptureBridge;
    #[allow(async_fn_in_trait)]
    async fn capture_stdout_json_and_result(self) -> (StdOutCaptureBridge, F::Output);
}

impl<F> CaptureStdoutExt<F> for F
where
    F: Future,
{
    async fn capture_stdout_lines(self) -> Vec<String> {
    ...
    async fn capture_stdout_json_and_result(self) -> (StdOutCaptureBridge, F::Output) {
        ...
        (StdOutCaptureBridge::new(stdout_log.lines()), result)
    }
}
