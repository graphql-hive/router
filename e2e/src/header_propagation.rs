...
mod header_propagation_e2e_tests {
    ...
    #[ntex::test]
    async fn should_ignore_empty_header_value_when_propagating() {
        let subgraphs = TestSubgraphs::builder().build().start().await;
        let router = TestRouter::builder()
            .with_subgraphs(&subgraphs)
            .file_config("configs/header_propagation.router.yaml")
        ...
        let last_request = &subgraph_requests[0];
        assert!(
            last_request.headers.get("x-context").is_none(),
            "expected empty x-context header not to be propagated to subgraph"
        );
    }
}
