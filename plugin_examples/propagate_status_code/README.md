...
- In the `on_end` callback of `on_subgraph_http_request`, it checks the status code of the subgraph response and if it is a code defined in the configuration, it saves that status code in the context. If the existing code in the context is lower than the new one, it updates it.
