...
mod tests {
    ...
    async fn should_refresh_token_if_expired() {
        let mut refresh_endpoint_server = mockito::Server::new_async().await;
        ...
        let router = TestRouterBuilder::new()
            ...
            .inline_config(
                include_str!("../../../plugin_examples/jwt_cookie/router.config.yaml").replace(
                    "0.0.0.0:9876",
                    refresh_endpoint_server.host_with_port().as_str(),
                ),
            )
            .register_plugin::<crate::plugin::JwtCookiePlugin>()
            .build()
            .start()
            .await;
        ...
        // Check cookies
        let set_cookie_headers = res.headers().get_all("set-cookie");
        ...
        assert!(
            found_jwt_token_in_subgraph_request,
            "Expected to find the new JWT token in the Authorization header of the request forwarded to the subgraph"
        );
    }
}
