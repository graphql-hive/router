...
- If the given expiry time of the token is in the past, or within the next 7 days, we call the refresh endpoint to get a new token, and add the new token to the plugin context
...
- `jwt_expired_at`: the expiry time of the token, which is used to determine if we need to refresh the token
