...
impl EnvVarOverrides {
    pub fn apply_overrides<T: BuilderState>(
        ...
        if let Some(hive_console_cdn_endpoint) = self.hive_console_cdn_endpoint.take() {
            config =
                config.set_override("supergraph.endpoint", hive_console_cdn_endpoint.clone())?;

            config = config.set_override(
                "persisted_documents.source.hive.endpoint",
                hive_console_cdn_endpoint,
            )?;
            ...
            if let Some(hive_console_cdn_key) = self.hive_console_cdn_key.take() {
                ...
                config = config
                    .set_override("persisted_documents.source.hive.key", hive_console_cdn_key)?;
            } else {
