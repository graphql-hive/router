...
impl RequestIdentifierExtractor {
    ...
    pub fn extract_req_id(&self, request: &HttpRequest) -> RequestIdentifier {
        if let Some(req_id_header) = request
            .headers()
            .get(self.cfg.id_header.get_header_ref())
            .and_then(|v| v.to_str().ok())
        {
            return RequestIdentifier::FromRequest(req_id_header.to_string());
        }
...
impl Display for RequestIdentifier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RequestIdentifier::FromRequest(value) => write!(f, "{}", value),
