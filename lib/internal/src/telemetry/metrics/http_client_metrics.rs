...
impl Capture<HttpClientRequestState<'_>> {
    pub fn finish_success(&mut self, response_body_size: u64) {
        self.record(Some(response_body_size), None);
    }

    pub fn finish_error(&mut self, error_type: &'static str) {
    ...
    fn record(&mut self, response_body_size: Option<u64>, error_type: Option<&str>) {
