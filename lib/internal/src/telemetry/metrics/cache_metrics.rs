...
impl CacheMetricSet {
    ...
    pub fn hit(&self, duration: std::time::Duration) {
        self.record_request(CacheResult::Hit, duration);
    }

    pub fn miss(&self, duration: std::time::Duration) {
        self.record_request(CacheResult::Miss, duration);
    }
    ...
    fn record_request(&self, result: CacheResult, duration: std::time::Duration) {
        if !self.instruments.is_enabled() {
            return;
        }
...
impl Capture<CacheRequestState<'_>> {
    pub fn finish_hit(self) {
        self.record(CacheResult::Hit);
    }

    pub fn finish_miss(self) {
        self.record(CacheResult::Miss);
    }
