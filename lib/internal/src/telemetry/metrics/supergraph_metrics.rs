...
use crate::telemetry::metrics::{
    labels::{self, SupergraphPollResult, SupergraphProcessStatus},
    names, Capture,
};
...
impl<'a> Capture<SupergraphPollState<'a>> {
    pub fn finish_not_modified(self) {
        self.record(SupergraphPollResult::NotModified);
    }

    pub fn finish_updated(self) {
        self.record(SupergraphPollResult::Updated);
    }

    pub fn finish_error(self) {
        self.record(SupergraphPollResult::Error);
    }

    fn record(self, result: SupergraphPollResult) {
...
impl<'a> Capture<SupergraphProcessState<'a>> {
    pub fn finish_ok(self) {
        self.record(SupergraphProcessStatus::Ok);
    }

    pub fn finish_error(self) {
        self.record(SupergraphProcessStatus::Error);
    }

    fn record(self, status: SupergraphProcessStatus) {
        let Some(state) = self.take() else {
            return;
        };
        ...
        if let Some(histogram) = &state.instruments.process_duration {
            histogram.record(
                ...
                &[KeyValue::new(labels::STATUS, status.as_str())],
            );
        }
    }
}
