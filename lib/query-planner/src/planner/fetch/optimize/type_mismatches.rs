...
impl FetchGraph {
    ...
    pub(crate) fn fix_conflicting_type_mismatches(
        ...
        for (node_index, mismatches_paths) in pending_patches {
            ...
            for (root_type_name, mismatch_path) in mismatches_paths {
                    ...
                    mismatch_path.last()
                {
                    ...
                    let mut selections = node
                        .output_new
                        .selections_for_definition_mut(&root_type_name);

                    if let Some(selection_set) =
                        find_selection_set_by_path_mut(&mut selections, lookup_path)
                    {
