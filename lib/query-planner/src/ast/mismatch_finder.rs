...
impl<'a> SelectionMismatchFinder<'a> {
    ...
    pub fn find_mismatches_in_node(
        ...
        selection_sets: &FetchStepSelections,
    ) -> MismatchesFound {
        ...
        for (root_type_name, selection_set) in selection_sets.iter() {
            if selection_set.is_empty() {
                continue;
            }

            println!(
                "root_type_name: {}, selection_set: {}",
                root_type_name, selection_set
            );

            let entrypoint_type = subgraph_state.definitions.get(root_type_name).unwrap();
    ...
    if let Some(maybe_conflicting_type) = encountered_field_to_type.get(field_name) {
        if !maybe_conflicting_type.can_be_merged_with(field_type) {
            ...
            if !left_is_composite || !right_is_composite {
                ...
                mismatches_found.push((root_type_name.to_string(), field_path.clone()));
            }
        }
    } else {
