...
use query_planner::planner::walker::{walk_operation, WalkContext};
...
fn query_plan_pipeline(c: &mut Criterion) {
    ...
    c.bench_function("query_plan", |b| {
        b.iter(|| {
            ...
            let bb_ctx = black_box(&ctx);
            let bb_operation = black_box(&operation);
            let bb_supergraph_state = black_box(&supergraph_state);

            let best_paths_per_leaf = walk_operation(bb_ctx, bb_operation)
                .expect("walk_operation failed during benchmark");
            let query_tree = find_best_combination(bb_ctx, best_paths_per_leaf).unwrap();
            let fetch_graph = build_fetch_graph_from_query_tree(bb_ctx.graph, query_tree).unwrap();
            let query_plan =
