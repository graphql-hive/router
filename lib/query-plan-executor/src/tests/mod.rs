...
use crate::{executors::local::LocalSubgraphExecutor, SubgraphExecutorMap};
...
fn query_executor_pipeline_locally() {
    tokio_test::block_on(async {
        ...
        subgraph_executor_map.insert(
            "accounts".to_string(),
            Arc::new(Box::new(LocalSubgraphExecutor::new(accounts))),
        );
        subgraph_executor_map.insert(
            "inventory".to_string(),
            Arc::new(Box::new(LocalSubgraphExecutor::new(inventory))),
        );
        subgraph_executor_map.insert(
            "products".to_string(),
            Arc::new(Box::new(LocalSubgraphExecutor::new(products))),
        );
        subgraph_executor_map.insert(
            "reviews".to_string(),
            Arc::new(Box::new(LocalSubgraphExecutor::new(reviews))),
        );
