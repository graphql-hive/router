...
use crate::{execution_result::ExecutionResult, executors};

use crate::execution_request::ExecutionRequest;
use executors::common::SubgraphExecutor;

struct BatchLoadFn<'a> {
    executor: Arc<Box<dyn SubgraphExecutor + Send + Sync + 'a>>,
}

impl Hash for ExecutionRequest {
...
impl Eq for ExecutionRequest {}

impl PartialEq for ExecutionRequest {
...
impl BatchFn<ExecutionRequest, ExecutionResult> for BatchLoadFn<'_> {
    async fn load(
        &mut self,
        keys: &[ExecutionRequest],
    ) -> HashMap<ExecutionRequest, ExecutionResult> {
        if keys.len() == 1 {
            let request = &keys[0];
            let result = self.executor.execute(request.clone()).await;
            let mut result_map: HashMap<ExecutionRequest, ExecutionResult> = HashMap::new();
...
fn split_result(result: &ExecutionResult, num_results: usize) -> Vec<ExecutionResult> {
