...
pub enum SubgraphExecutorError {
    ...
    #[error("Subgraph '{0}' responded with an invalid HTTP status code {1}")]
    #[strum(serialize = "SUBREQUEST_HTTP_ERROR")]
    InvalidHttpStatusCode(String, u16, Arc<HeaderMap>),
    #[error(transparent)]
...
impl SubgraphExecutorError {
    ...
    pub fn response_headers(&self) -> Option<&http::HeaderMap> {
        match self {
            ...
            Self::InvalidHttpStatusCode(_, _, headers) => Some(headers.as_ref()),
    ...
    pub fn error_extensions(&self) -> Option<BTreeMap<String, Value>> {
        match self {
            Self::InvalidHttpStatusCode(_, status_code, _) => Some(BTreeMap::from([(
                "http".to_string(),
