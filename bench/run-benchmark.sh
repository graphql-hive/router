...
parse_wrk_output() {
  read -r RATE_RPS STATUS_FAILURES GRAPHQL_ERRORS <<EOF
$(awk '
  /^Requests\/sec:/ { rate=$2 }
...
if ! [[ "$STATUS_FAILURES" =~ ^[0-9]+$ ]] || ! [[ "$GRAPHQL_ERRORS" =~ ^[0-9]+$ ]]; then
  echo "Could not parse validation counters from wrk output."
  exit 1
fi

VALIDATION_TOTAL_FAILURES=$((STATUS_FAILURES + GRAPHQL_ERRORS))
