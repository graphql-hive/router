...
|[**circuit\_breaker**](#telemetryhiveusage_reportingcircuit_breaker)|`object`, `null`|Optional tuning of the circuit breaker applied to outbound calls to Hive Console. When omitted, the SDK uses its default settings (50% error threshold, rolling sample of 5, 30s reset timeout, 10 half-open probes).<br/>||
