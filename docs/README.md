...
|**reject\_unauthenticated\_requests**|`boolean`, `null`|If set to `true`, the entire request will be rejected if the JWT token is not present in the request.<br/>|no|
...
|**source**|`string`|Constant Value: `"local"`<br/>|yes|
...
|Name|Type|Description|Required|
|----|----|-----------|--------|
|**cache\_duration**|`string`|Duration after which the cached JWKS should be expired. If not specified, the default value will be used.<br/>Default: `"10m"`<br/>|no|
...
|----|----|-----------|--------|
|**name**|`string`||yes|
|**source**|`string`|Constant Value: `"query_param"`<br/>|yes|
