...
|[**router**](#traffic_shapingrouter)|`object`|Configuration for the router itself, e.g., for handling incoming requests, or other router-level traffic shaping configurations.<br/>Default: `{"dedupe_enabled":true,"request_timeout":"1m"}`<br/>||
...
|Name|Type|Description|Required|
|----|----|-----------|--------|
|**dedupe\_enabled**|`boolean`|Enables/disables in-flight request deduplication at the router endpoint level.<br/><br/>When enabled, identical incoming GraphQL query requests that are processed at the same time<br/>share the same in-flight execution result.<br/>Default: `true`<br/>||
